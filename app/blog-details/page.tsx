...
export default function BlogDetails() {
    ...
    useEffect(() => {
        if (id) {
            const currentIndex = data.findIndex((post: BlogPost) => String(post.id) === id)
            if (currentIndex !== -1) {
                setBlogPost(data[currentIndex])
            ...
            // If no ID provided, show the first post by default
            if (data.length > 0) {
                setBlogPost(data[0])
                if (data.length > 1) {
                    setNextPost(data[1])
                ...
                // Set latest posts based on date (excluding first post)
                const sortedPosts = [...data].sort((a, b) => {
                    return new Date(b.date).getTime() - new Date(a.date).getTime();
                });
                
                const latest = sortedPosts
                    .filter(post => post.id !== data[0].id)
                    .slice(0, 3);
    ...
    // If no blog post is found, show a loading state
    if (!blogPost) {
        return (
                                ...
                                <div className="blog__details-wrap mb-100">
                                    <div className="blog__details-thumb">
                                        ...
                                        <div className="blog__post-meta">
                                            <ul className="list-wrap">
                                                <li><i className="far fa-user" /><Link href="/blog-details">{blogPost.author}</Link></li>
                                    ...
                                    <div className="blog__details-content">
                                        ...
                                        <div className="inner__page-nav pt-30">
                                            {prevPost && (
                                                <Link href={`/blog-details?id=${prevPost.id}`} className="nav-btn text-md-end">
                                            ...
                                            {nextPost && (
                                                <Link href={`/blog-details?id=${nextPost.id}`} className="nav-btn">
                                                    <div className="text-wrap mb-0">
                                                        <span>
                                                            VIEW NEXT
                                                        </span>
                                            ...
                                            {latestPosts.map((post, index) => (
                                                <div className="rc-post-item" key={index}>
                                                    ...
                                                    <div className="content">
                                                        <h4 className="title">
                                                            <Link href={`/blog-details?id=${post.id}`}>{post.title}</Link>
                                                        </h4>
